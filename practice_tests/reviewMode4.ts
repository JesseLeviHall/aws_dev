...
Amazon RDS supports using Transparent Data Encryption (TDE) to encrypt stored data on your DB instances running Microsoft SQL Server. TDE automatically encrypts data before it is written to storage, and automatically decrypts data when the data is read from storage. RDS also supports enhanced monitoring. Whereas detailed monitoring is for ec2 logs to catch cpu and memory metrics. Then, custom metrics can be high-res or standard.  High-res can be 1 sec. Standard is 1minute.  The granularity determines the level of detail in the metric data and how often CloudWatch Logs sends the data to CloudWatch Metrics for aggregation. 
